// hero-movies.js — Кино Hero логик (setHero, initHero) — хурдасгасан
...
window.setHero = async (i) => {
  ...
  // ── Poster ТЭРХЭНД харуул — trailer хүлээхгүй ─────────────
  showPoster(m.poster);
  window.hideVolBtn();
  window._heroMuted = true;
  window.updateVolBtn(true);

  // ── Trailer async — hero-г хаахгүй ────────────────────────
...
async function _loadTrailerAsync(m, slideIndex) {
  ...
  if (!trailerUrl && m.tmdbId) {
    ...
    // Cache-д хадгалах (дараагийн slide-д дахин татахгүй)
    if (trailerUrl) m.trailer = trailerUrl;
  }

  // Хэрэглэгч өөр slide рүү шилжсэн бол зогсоох
